...
async function posCompleteCheckout(cart,customerId,method) { const subtotal=cart.reduce((sum,item)=>sum+(item.isWholesale?item.wholesalePrice:item.price)*item.qty,0);const total=subtotal*(1+Number(S.storeSettings.taxRate||0)/100);S.posPendingOrderId=S.posPendingOrderId||(globalThis.crypto?.randomUUID?.()||`${Date.now()}-0000-4000-8000-${Math.random().toString(16).slice(2).padEnd(12,'0').slice(0,12)}`);const data=await posApiFetch('/pos/checkout',{method:'POST',body:{client_order_id:S.posPendingOrderId,customer_id:customerId||null,payments:[{method,amount:Number(total.toFixed(2)),reference:S.mobileTxId||null}],items:cart.map(item=>({product_id:item.id,quantity:item.qty,wholesale:Boolean(item.isWholesale),discount_percent:Number(item.discountPercent||0)})),mobile_phone:S.mobilePhone||null,payment_reference:S.mobileTxId||null}});const mapped=mapTransaction({...data,id:data.id,total_amount:data.total_amount,items_count:cart.reduce((n,item)=>n+item.qty,0),customer_name:POS_CUSTOMERS.find(item=>item.id===customerId)?.name,cashier_name:S.posActiveUser?.name});const existing=POS_TRANSACTIONS.findIndex(item=>item._backendId===mapped._backendId);if(existing>=0)POS_TRANSACTIONS[existing]=mapped;else POS_TRANSACTIONS.unshift(mapped);S.posPendingOrderId=null;return data; }
