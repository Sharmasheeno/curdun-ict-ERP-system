...
function renderPOSAuditLogs() {
  const f=S.posAuditFilters||{}; const data=S.posAuditData||{}; const rows=data.items||[];
  const labels={amount:'Amount',total:'Total',reference:'Reference',reason:'Reason',session_id:'Session',approval_id:'Approval ID',approved_action:'Approved action',approved_by:'Approving manager',original_order_id:'Original order',payments:'Payment methods',changed_fields:'Changed fields'};
  const friendly=value=>typeof value==='boolean'?(value?'ON':'OFF'):(value===null?'Not set':(typeof value==='object'?JSON.stringify(value):String(value)));
  const printable=value=>Array.isArray(value)?value.map(v=>typeof v==='object'?(v.method?`${v.method}: $${Number(v.amount||0).toFixed(2)}`:JSON.stringify(v)):String(v)).join(', '):(typeof value==='object'&&value!==null?Object.entries(value).map(([key,v])=>{const name=key.replaceAll('_',' ').replace(/\b\w/g,c=>c.toUpperCase());return v&&typeof v==='object'&&Object.hasOwn(v,'before')?`${name} - Before: ${friendly(v.before)}; After: ${friendly(v.after)}`:`${name}: ${friendly(v)}`;}).join(' | '):String(value));
  const details=row=>{const pairs=Object.entries(row.details||{}).filter(([key,value])=>labels[key]&&value!==null&&value!=='');return `<details class="audit-details"><summary>View details</summary><div class="audit-detail-grid">${row.entity?.reference?`<div><strong>Reference</strong><span>${esc(row.entity.reference)}</span></div>`:''}${row.approving_manager?`<div><strong>Approving Manager</strong><span>${esc(row.approving_manager)}</span></div>`:''}${row.historical_action?`<div><strong>Historical Action</strong><span>${esc(row.historical_action)}</span></div>`:''}${pairs.map(([key,value])=>`<div><strong>${esc(labels[key])}</strong><span>${esc(printable(value))}</span></div>`).join('')}${row.before&&Object.keys(row.before).length?`<div><strong>Before</strong><span>${esc(JSON.stringify(row.before))}</span></div>`:''}</div></details>`;};
  const body=rows.length?rows.map(row=>`<tr><td>${esc(row.date||'-')}</td><td><span class="pill pill-gold">${esc(row.action||'-')}</span></td><td><span class="pill ${row.result==='SUCCESS'?'pill-green':'pill-red'}">${esc(row.result||'-')}</span></td><td>${esc(row.account_user||'-')}</td><td>${esc(row.pos_employee||'-')}</td><td>${esc(row.pos||'-')}</td><td>${esc(row.session||'-')}</td><td>${esc(row.entity?.type||'Entity')} #${esc(row.entity?.id||'-')}${row.entity?.reference?`<div style="font-size:11px;color:var(--text-muted)">${esc(row.entity.reference)}</div>`:''}</td></tr><tr class="audit-detail-row"><td colspan="8">${details(row)}</td></tr>`).join(''):'<tr><td colspan="8" style="text-align:center;color:var(--text-muted)">No audit records match these filters.</td></tr>';
  return `<div class="page-header"><div><h2 class="page-title">Audit Logs</h2><div class="page-subtitle">Company-scoped operational history with account and POS employee identity.</div></div></div><div class="data-section" style="padding:14px;margin-bottom:14px"><div class="crud-grid-2" style="grid-template-columns:repeat(4,minmax(140px,1fr))">${[['date_from','Date from','date'],['date_to','Date to','date'],['action','Action','text'],['result','Result','text'],['account_user','Account User','text'],['pos_employee','POS Employee','text'],['pos','POS','text'],['session','Session','text'],['entity','Entity ID','text']].map(([key,label,type])=>`<div class="form-group"><label class="form-label">${label}</label><input class="form-input pos-audit-filter" data-audit-filter="${key}" type="${type}" value="${esc(f[key]||'')}"/></div>`).join('')}</div><div style="display:flex;gap:8px"><button class="btn btn-primary btn-sm" id="btn-audit-run" ${S.posAuditLoading?'disabled':''}>${S.posAuditLoading?'Loading...':'Apply filters'}</button><button class="btn btn-outline btn-sm" id="btn-audit-clear">Clear</button></div>${S.posAuditError?`<div class="crud-error" style="margin-top:10px">${esc(S.posAuditError)}</div>`:''}</div><div class="data-section"><div class="section-header-bar"><h3 class="chart-title">POS audit trail</h3><span class="ml-auto" style="font-size:12px;color:var(--text-muted)">${Number(data.total||0)} matching records - page ${Number(data.page||1)} of ${Number(data.pages||1)}</span></div><div class="overflow-x-auto"><table class="data-table" style="min-width:1050px"><thead><tr><th>Date</th><th>Action</th><th>Result</th><th>Account User</th><th>POS Employee</th><th>POS</th><th>Session</th><th>Entity</th></tr></thead><tbody>${body}</tbody></table></div><div style="display:flex;justify-content:flex-end;gap:8px;padding:12px"><button class="btn btn-outline btn-sm" id="btn-audit-prev" ${Number(data.page||1)<=1?'disabled':''}>Previous</button><button class="btn btn-outline btn-sm" id="btn-audit-next" ${Number(data.page||1)>=Number(data.pages||1)?'disabled':''}>Next</button></div></div>`;
...
function wirePOSEvents() {
  ...
  document.getElementById('btn-audit-run')?.addEventListener('click', async () => {
    ...
    try { S.posAuditData=await posLoadAuditLogs({...S.posAuditFilters,page:1,per_page:50}); }
  ...
  const loadAuditPage=async page=>{S.posAuditLoading=true;S.posAuditError='';render();try{S.posAuditData=await posLoadAuditLogs({...S.posAuditFilters,page,per_page:50});}catch(error){S.posAuditError=error.message||'Audit logs could not be loaded.';}finally{S.posAuditLoading=false;render();}};
  document.getElementById('btn-audit-prev')?.addEventListener('click',()=>loadAuditPage(Math.max(1,Number(S.posAuditData?.page||1)-1)));
  document.getElementById('btn-audit-next')?.addEventListener('click',()=>loadAuditPage(Number(S.posAuditData?.page||1)+1));
